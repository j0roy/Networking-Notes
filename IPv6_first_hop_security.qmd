...
IPv6 snooping: Build from DHCPv6 and NDP messages. (similar to DAI) Build neighbor binding table through IPv6 neighbor tracking. Prevent host spoofing on the segment On Vlan or on interface.

RA Guard: Prevent spoofing of router and prefix on the segment. On Vlan or on interface.

DHCPv6 Guard: Prevent DHCPv6 server spoofing On Vlan or on interface.

IPv6 Source-gurad: Data plane filter. Relies on IPv6 snooping to create the binding table, Creates automatic IPv6 PACL to filter sources base on binding table.
